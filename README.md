...
- 🔊 **多引擎 TTS**：優先使用 Azure Speech，退回 ElevenLabs，最終使用 Edge-TTS；若 Edge 無法連線且在 macOS 上執行，會改用系統內建的 `say` 指令輸出語音。
...
- macOS 內建語音備援可用 `MORNINGCAST_NATIVE_TTS_VOICE` 環境變數覆寫使用的 `say` 聲線（預設依 Edge 語系選擇）。
- 請確認歌曲音檔具有使用授權。
